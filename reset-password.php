...
if( !$token OR empty($token) ){
    ...
    header('Location: token-request.php');
    exit();
}
...
<html lang="en">
  <head>
   ...
   <title>User Password Reset | <?php echo SITE_NAME; ?></title>
  ...
  try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['resetPassword'])) {
        ...
        $passwordVerify = filter_var($_POST['password-verify'], FILTER_SANITIZE_STRING);

   
        $user = new classes\user();
        $user -> resetPassword($email, $password, $passwordVerify, $token); 
...
?>
          <form action="" method="POST">
                ...
                <div data-mdb-input-init class="form-outline mb-4">
                <label class="form-label" for="form3Example99">New password</label>
                ...
                <div class="d-flex justify-content-end pt-3">
                  ...
                  class="btn btn-warning btn-lg ms-2" name="resetPassword">Submit form</button>
                </div>
                </form> 
              </div>
            </div>
